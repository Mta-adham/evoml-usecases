...
def main():
    ...
    file_name = "all-users-daily-data"
    label_name = "energy"
    index_name = "date"
    tags = ["sample"]
    ...
    full_path = base_dir / f"datasets/standard/timeseries_groupby/{file_name}.csv"
    ...
    # Group by 'userId' and create lagged columns
    def lag_group(group):
        for lag in range(1, n_lags + 1):
            group[f'lag_{lag}'] = group[label_name].shift(lag)
        return group

    # Check if the Date column is sequential
    def is_sequential(date_column):
        diffs = date_column.diff().dropna()  # Compute differences between consecutive dates
        return (diffs == diffs.iloc[0]).all()  # Check if all differences are the same

    # Apply the lagging function to each group
    lagged_df = df.groupby('userId').apply(lag_group).reset_index(drop=True)

    print(is_sequential(df[index_name]))

    # Get trial configuration
    default_config, config = get_trial_configuration(
        file_name, lagged_df, task, label_name, tags, is_timeseries
    )

    default_config.options.timeSeriesWindowSize = 0
    default_config.options.validationMethodOptions.holdoutOptions.keepOrder = True
